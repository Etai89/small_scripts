...
def refresh_balances():
    ...
    bank_balance_label.config(text=f"Bank Balance: ${bank_balance:.2f}", font=("Arial", 12, "bold"), justify="right", fg="#008000" if bank_balance >= 0 else "#FF0000")
    cash_balance_label.config(text=f"Cash Balance: ${cash_balance:.2f}", font=("Arial", 12, "bold"), justify="right", fg="#008000" if cash_balance >= 0 else "#FF0000")
...
def show_history():
    ...
    for row in rows:
        amount_color = "#008000" if row[2] == 'Deposit' else "#FF0000"  # Green for Deposit, Red for Withdrawal
        history_text.insert(tk.END, f"Transaction ID: {row[0]}, Date: {row[1]}, Action: {row[2]}, Amount: ${row[3]:.2f}, Comment: {row[4]}\n", ('green' if row[2] == 'Deposit' else 'red'))
...
# Deposit Section
deposit_frame = tk.LabelFrame(root, text="Deposit", font=("Arial", 12))
...
# Withdraw Section
withdraw_frame = tk.LabelFrame(root, text="Withdraw", font=("Arial", 12))
...
# Delete Section
delete_frame = tk.LabelFrame(root, text="Delete Transaction", font=("Arial", 12))
...
# Balance Section
balance_frame = tk.LabelFrame(root, text="Balances", font=("Arial", 12))
